...
namespace ForeverNote.Services.Tests.Authentication
{
    [TestClass()]
    public class CookieAuthenticationServiceTests
    {
        ...
        public async Task GetAuthenticatedCustomer_UsernameEnableRegisterd_ReturnCustomer()
        {
            ...
            var cliaim = new Claim(ClaimTypes.Name, "Johny","", ForeverNoteCookieAuthenticationDefaults.ClaimsIssuer);
            IList <Claim> claims = new List<Claim>
            {
                 new Claim(ClaimTypes.Name,ClaimTypes.Name,"", ForeverNoteCookieAuthenticationDefaults.ClaimsIssuer)
              };
            var principals = new ClaimsPrincipal(new ClaimsIdentity(claims, ForeverNoteCookieAuthenticationDefaults.AuthenticationScheme));
            _authServiceMock.Setup(c => c.AuthenticateAsync(It.IsAny<HttpContext>(), It.IsAny<string>()))
