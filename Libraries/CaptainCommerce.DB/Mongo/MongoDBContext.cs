    ...
    public class MongoDBContext : IDatabaseContext
    {
        ...
        public async Task CreateIndex<T>(IRepository<T> repository, OrderBuilder<T> orderBuilder, string indexName, bool unique = false) where T : BaseEntity
        {
            ...
            foreach (var (selector, value, fieldName) in orderBuilder.Fields)
                if (selector != null)
                    if (value)
                        keys.Add(Builders<T>.IndexKeys.Ascending(selector));
                    else
                        keys.Add(Builders<T>.IndexKeys.Descending(selector));
                else
                    if (value)
                    keys.Add(Builders<T>.IndexKeys.Ascending(fieldName));
                else
                    keys.Add(Builders<T>.IndexKeys.Descending(fieldName));

            try
            {
                _ = await ((MongoRepository<T>)repository).Collection.Indexes.CreateOneAsync(new CreateIndexModel<T>(Builders<T>.IndexKeys.Combine(keys),
