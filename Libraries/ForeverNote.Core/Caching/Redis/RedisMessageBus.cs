...
namespace ForeverNote.Core.Caching.Redis
{
    public class RedisMessageBus : IMessageBus
    {
        private readonly ISubscriber _subscriber;
        private readonly IServiceProvider _serviceProvider;
        private readonly ForeverNoteConfig _foreverNoteConfig;

        private static readonly string _clientId = Guid.NewGuid().ToString("N");

        public RedisMessageBus(ISubscriber subscriber, IServiceProvider serviceProvider, ForeverNoteConfig foreverNoteConfig)
        {
            _subscriber = subscriber;
            _serviceProvider = serviceProvider;
            _foreverNoteConfig = foreverNoteConfig;
            SubscribeAsync();
        }
        ...
        public async Task PublishAsync<TMessage>(TMessage msg) where TMessage : IMessageEvent
        {
            try
            {
                ...
                await pub.PublishAsync(_foreverNoteConfig.RedisPubSubChannel, message);
            }
            catch(Exception ex)
            {
        ...
        public virtual Task SubscribeAsync()
        {
            ...
            sub.SubscribeAsync(_foreverNoteConfig.RedisPubSubChannel, async (_, redisValue) =>
            {
                try
                {
