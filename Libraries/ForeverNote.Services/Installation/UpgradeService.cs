...
namespace ForeverNote.Services.Installation
{
    public partial class UpgradeService : IUpgradeService
    {
        ...
        public UpgradeService(IServiceProvider serviceProvider,
            IMediator mediator,
            IRepository<Core.Domain.Common.ForeverNoteVersion> versionRepository)
        {
        ...
        public virtual async Task UpgradeData(string fromversion, string toversion)
        {
            ...
            if (fromversion == toversion)
            {
                ...
                if (databaseversion != null)
                {
                    databaseversion.DataBaseVersion = Core.ForeverNoteVersion.CurrentVersion;
                    await _versionRepository.UpdateAsync(databaseversion);
                }
                else
                {
                    databaseversion = new Core.Domain.Common.ForeverNoteVersion {
        ...
        private async Task From400To410()
        {
            ...
            var _keepAliveScheduleTask = keepliveTask.Table.Where(x => x.Type == "ForeverNote.Services.Tasks.KeepAliveScheduleTask").FirstOrDefault();
        ...
        private async Task From430To440()
        {
            ...
            var messagetemplateService = _serviceProvider.GetRequiredService<ForeverNote.Services.Messages.IMessageTemplateService>();
        ...
        private async Task From440To450()
        {
            ...
            foreach (var task in tasks.Table)
            {
                ...
                if (task.Type == "ForeverNote.Services.Tasks.ClearLogScheduleTask")
                {
                    task.Type = "ForeverNote.Services.Tasks.ClearLogScheduleTask, ForeverNote.Services";
                    await tasks.UpdateAsync(task);
                }
            }
            #endregion
