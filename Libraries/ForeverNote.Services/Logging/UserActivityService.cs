...
namespace ForeverNote.Services.Logging
{
    ...
    public class UserActivityService : IUserActivityService
    {
        ...
        /// Key for caching
        /// </summary>
        private const string ACTIVITYTYPE_ALL_KEY = "ForeverNote.activitytype.all";
        /// <summary>
        /// Key pattern to clear cache
        /// </summary>
        private const string ACTIVITYTYPE_PATTERN_KEY = "ForeverNote.activitytype.";
        ...
        {
            await InsertActivity(systemKeyword, entityKeyId, comment, _workContext.CurrentUser, commentParams);
        }
        ...
        /// <param name="pageSize">Page size</param>
        /// <returns>Activity log items</returns>
        public virtual async Task<IPagedList<ActivityLog>> GetNotebookActivities(DateTime? createdOnFrom = null,
            DateTime? createdOnTo = null, string notebookId = "", int pageIndex = 0, int pageSize = int.MaxValue)
        {
            ...
            var activityTypes = await GetAllActivityTypesCached();
            var activityTypeIds = activityTypes.ToList().Where(at => _activityKeywordsProvider.GetNotebookSystemKeywords().Contains(at.SystemKeyword)).Select(x => x.Id);

            query = query.Where(al => activityTypeIds.Contains(al.ActivityLogTypeId));

            query = query.Where(al => al.EntityKeyId == notebookId);
            query = query.OrderByDescending(al => al.CreatedOnUtc);
            return await PagedList<ActivityLog>.Create(query, pageIndex, pageSize);
        }
        ...
        /// <param name="pageSize">Page size</param>
        /// <returns>Activity log items</returns>
        public virtual async Task<IPagedList<ActivityLog>> GetKnowledgebaseNotebookActivities(DateTime? createdOnFrom = null,
            DateTime? createdOnTo = null, string notebookId = "", int pageIndex = 0, int pageSize = int.MaxValue)
        {
            var query = _activityLogRepository.Table;
            if (createdOnFrom.HasValue)
            ...
            var activityTypes = await GetAllActivityTypesCached();
            var activityTypeIds = activityTypes.ToList().Where(at => _activityKeywordsProvider.GetKnowledgebaseNotebookSystemKeywords().Contains(at.SystemKeyword)).Select(x => x.Id);
        ...
        /// <param name="createdOnFrom">Log item creation from; null to load all records</param>
        /// <param name="createdOnTo">Log item creation to; null to load all records</param>
        /// <param name="notebookId">Manufacturer identifier</param>        
        /// <param name="pageIndex">Page index</param>
        /// <param name="pageSize">Page size</param>
        /// <returns>Activity log items</returns>
        ...
        /// <param name="pageSize">Page size</param>
        /// <returns>Activity log items</returns>
        public virtual async Task<IPagedList<ActivityLog>> GetNoteActivities(DateTime? createdOnFrom = null,
            DateTime? createdOnTo = null, string noteId = "", int pageIndex = 0, int pageSize = int.MaxValue)
        {
            ...
            var activityTypes = await GetAllActivityTypesCached();
            var activityTypeIds = activityTypes.ToList().Where(at => _activityKeywordsProvider.GetNoteSystemKeywords().Contains(at.SystemKeyword)).Select(x => x.Id);

            query = query.Where(al => activityTypeIds.Contains(al.ActivityLogTypeId));

            query = query.Where(al => al.EntityKeyId == noteId);
            query = query.OrderByDescending(al => al.CreatedOnUtc);
            return await PagedList<ActivityLog>.Create(query, pageIndex, pageSize);
        }
