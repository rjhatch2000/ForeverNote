...
using ForeverNote.Services.Affiliates;
    ...
    public partial class OrderProcessingService : IOrderProcessingService
    {
        ...
        protected virtual async Task<ProcessPaymentResult> PrepareProcessPaymentResult(ProcessPaymentRequest processPaymentRequest, PlaceOrderContainter details)
        {
            ...
            if (!skipPaymentWorkflow)
            {
                ...
                if (paymentMethod == null)
                    throw new ForeverNoteException("Payment method couldn't be loaded");

                //ensure that payment method is active
                if (!paymentMethod.IsPaymentMethodActive(_paymentSettings))
                    throw new ForeverNoteException("Payment method is not active");
                    ...
                    if (details.IsRecurringShoppingCart)
                    {
                        ...
                        switch (recurringPaymentType)
                        {
                            case RecurringPaymentType.NotSupported:
                                throw new ForeverNoteException("Recurring payments are not supported by selected payment method");
                            case RecurringPaymentType.Manual:
                            case RecurringPaymentType.Automatic:
                                processPaymentResult = await _paymentService.ProcessRecurringPayment(processPaymentRequest);
                                break;
                            default:
                                throw new ForeverNoteException("Not supported recurring payment type");
                        }
                    }
                    else
                    {
                ...
                else
                {
                    ...
                    {
                        throw new ForeverNoteException("No recurring products");
                    }
                }
            }
            else
            ...
            if (processPaymentResult == null)
                throw new ForeverNoteException("processPaymentResult is not available");
        ...
        protected virtual async Task<PlaceOrderContainter> PreparePlaceOrderDetails(ProcessPaymentRequest processPaymentRequest)
        {
            ...
            if (details.Customer.IsGuest() && !_orderSettings.AnonymousCheckoutAllowed)
                throw new ForeverNoteException("Anonymous checkout is not allowed");

            //billing address
            if (details.Customer.BillingAddress == null)
                throw new ForeverNoteException("Billing address is not provided");

            if (!CommonHelper.IsValidEmail(details.Customer.BillingAddress.Email))
                throw new ForeverNoteException("Email is not valid");
            ...
            if (!String.IsNullOrEmpty(details.BillingAddress.CountryId))
            {
                ...
                if (country != null)
                    if (!country.AllowsBilling)
                        throw new ForeverNoteException(string.Format("Country '{0}' is not allowed for billing", country.Name));
            }
            ...
            if (!details.Cart.Any())
                throw new ForeverNoteException("Cart is empty");
            ...
            if (warnings.Any())
            {
                ...
                throw new ForeverNoteException(warningsSb.ToString());
            }
            ...
            if (!minOrderSubtotalAmountOk)
            {
                ...
                throw new ForeverNoteException(string.Format(_localizationService.GetResource("Checkout.MinOrderSubtotalAmount"), _priceFormatter.FormatPrice(minOrderSubtotalAmount, true, false)));
            }
            ...
            if (!minOrderTotalAmountOk)
            {
                ...
                throw new ForeverNoteException(string.Format(_localizationService.GetResource("Checkout.MinOrderTotalAmount"), _priceFormatter.FormatPrice(minOrderTotalAmount, true, false)));
            }
            ...
            if (shoppingCartRequiresShipping)
            {
                ...
                else
                {
                    if (details.Customer.ShippingAddress == null)
                        throw new ForeverNoteException("Shipping address is not provided");
                    ...
                    if (!String.IsNullOrEmpty(details.ShippingAddress.CountryId))
                    {
                        ...
                        if (country != null)
                            if (!country.AllowsShipping)
                                throw new ForeverNoteException(string.Format("Country '{0}' is not allowed for shipping", country.Name));
                    }
                }
            ...
            if (!orderShippingTotalInclTax.HasValue || !orderShippingTotalExclTax.HasValue)
                throw new ForeverNoteException("Shipping total couldn't be calculated");
            ...
            if (!orderTotal.HasValue)
                throw new ForeverNoteException("Order total couldn't be calculated");
            ...
            if (details.IsRecurringShoppingCart)
            {
                ...
                if (!string.IsNullOrEmpty(recurringCyclesError))
                    throw new ForeverNoteException(recurringCyclesError);
        ...
        public virtual async Task ProcessNextRecurringPayment(RecurringPayment recurringPayment)
        {
            ...
            try
            {
                if (!recurringPayment.IsActive)
                    throw new ForeverNoteException("Recurring payment is not active");

                var initialOrder = recurringPayment.InitialOrder;
                if (initialOrder == null)
                    throw new ForeverNoteException("Initial order could not be loaded");

                var customer = await _customerService.GetCustomerById(initialOrder.CustomerId);
                if (customer == null)
                    throw new ForeverNoteException("Customer could not be loaded");

                var nextPaymentDate = recurringPayment.NextPaymentDate;
                if (!nextPaymentDate.HasValue)
                    throw new ForeverNoteException("Next payment date could not be calculated");
                ...
                if (result.Success)
                {
                    if (result.PlacedOrder == null)
                        throw new ForeverNoteException("Placed order could not be loaded");
                ...
                else
                {
                    ...
                    throw new ForeverNoteException(error);
                }
            }
            catch (Exception exc)
            {
        ...
        public virtual async Task<IList<string>> Capture(Order order)
        {
            ...
            if (!await CanCapture(order))
                throw new ForeverNoteException("Cannot do capture for order.");
        ...
        public virtual async Task MarkOrderAsPaid(Order order)
        {
            ...
            if (!await CanMarkOrderAsPaid(order))
                throw new ForeverNoteException("You can't mark this order as paid");
        ...
        public virtual async Task<IList<string>> Refund(Order order)
        {
            ...
            if (!await CanRefund(order))
                throw new ForeverNoteException("Cannot do refund for order.");
        ...
        public virtual async Task RefundOffline(Order order)
        {
            ...
            if (!CanRefundOffline(order))
                throw new ForeverNoteException("You can't refund this order");
        ...
        public virtual async Task<IList<string>> PartiallyRefund(Order order, decimal amountToRefund)
        {
            ...
            if (!await CanPartiallyRefund(order, amountToRefund))
                throw new ForeverNoteException("Cannot do partial refund for order.");
        ...
        public virtual async Task PartiallyRefundOffline(Order order, decimal amountToRefund)
        {
            ...
            if (!CanPartiallyRefundOffline(order, amountToRefund))
                throw new ForeverNoteException("You can't partially refund (offline) this order");
        ...
        public virtual async Task<IList<string>> Void(Order order)
        {
            ...
            if (!await CanVoid(order))
                throw new ForeverNoteException("Cannot do void for order.");
        ...
        public virtual async Task VoidOffline(Order order)
        {
            ...
            if (!CanVoidOffline(order))
                throw new ForeverNoteException("You can't void this order");
