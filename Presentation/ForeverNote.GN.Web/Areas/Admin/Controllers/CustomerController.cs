        ...
        public async Task<IActionResult> SendEmail(CustomerModel model)
        {
            ...
            try
            {
                if (String.IsNullOrWhiteSpace(customer.Email))
                    throw new ForeverNoteException("Customer email is empty");
                if (!CommonHelper.IsValidEmail(customer.Email))
                    throw new ForeverNoteException("Customer email is not valid");
                if (String.IsNullOrWhiteSpace(model.SendEmail.Subject))
                    throw new ForeverNoteException("Email subject is empty");
                if (String.IsNullOrWhiteSpace(model.SendEmail.Body))
                    throw new ForeverNoteException("Email body is empty");
        ...
        public async Task<IActionResult> SendPm(CustomerModel model, [FromServices] ForumSettings forumSettings)
        {
            ...
            try
            {
                if (!forumSettings.AllowPrivateMessages)
                    throw new ForeverNoteException("Private messages are disabled");
                if (customer.IsGuest())
                    throw new ForeverNoteException("Customer should be registered");
                if (String.IsNullOrWhiteSpace(model.SendPm.Subject))
                    throw new ForeverNoteException("PM subject is empty");
                if (String.IsNullOrWhiteSpace(model.SendPm.Message))
                    throw new ForeverNoteException("PM message is empty");
