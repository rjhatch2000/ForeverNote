...
namespace ForeverNote.Web.Framework.Mvc.Filters
{
    /// <summary>
    ...
    public class SignOutFromExternalAuthenticationAttribute : TypeFilterAttribute
    {
        ...
        private class SignOutFromExternalAuthenticationFilter : IAuthorizationFilter
        {
            ...
            public async void OnAuthorization(AuthorizationFilterContext filterContext)
            {
                ...
                var authenticateResult = await filterContext.HttpContext.AuthenticateAsync(ForeverNoteCookieAuthenticationDefaults.ExternalAuthenticationScheme);
                if (authenticateResult.Succeeded)
                    await filterContext.HttpContext.SignOutAsync(ForeverNoteCookieAuthenticationDefaults.ExternalAuthenticationScheme);
            }
